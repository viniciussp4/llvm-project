...
static cl::opt<unsigned> TinyThreshold("size-inline-tiny-threshold", cl::Hidden,
...
static cl::opt<unsigned>
    TrivialNumAttempts("size-inline-trivial-attempts", cl::Hidden,
...
static bool TrivialInlining(Module &M) {
  ...
  std::sort(AllFunctions.begin(), AllFunctions.end(),
            [&](const Function *F1, const Function *F2) -> bool {
  ...
  for (Function *F : AllFunctions) {
    ...
    if (FSize > MaxSize)
      break; // functions are sorted

    if (F->getNumUses() > 0 &&
        (FSize <= TinyThreshold ||
         (F->getNumUses() == 1 && FSize <= SolitaryMaxThreshold))) {
...
ModulePass *llvm::createSizeInliningPass() { return new SizeInlining(); }
